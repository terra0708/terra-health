                            ...
                            <Box sx={{ display: 'flex', gap: 2 }}>
                                ...
                                >
                                    {settings.sources.map((s) => (
                                        <MenuItem key={s.id} value={s.value}>{s.label}</MenuItem>
                                    ))}
                                </TextField>
                                <TextField
                                    select
                                ...
                                <Select
                                    ...
                                    renderValue={(selected) => (
                                        <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
                                            {selected.map((val) => {
                                                const sColor = settings.services.find(s => s.name === val)?.color || theme.palette.primary.main;
                                                return <Chip key={val} label={val} size="small" sx={{ bgcolor: alpha(sColor, 0.1), color: sColor }} />;
                                            })}
                                        </Box>
                                    )}
                                >
                                    {settings.services.map((s) => <MenuItem key={s.id} value={s.name}>{s.name}</MenuItem>)}
                                </Select>
                            </FormControl>
                            ...
                            <FormControl fullWidth>
                                ...
                                <Select
                                    ...
                                    renderValue={(selected) => (
                                        <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
                                            {selected.map((val) => {
                                                const tColor = settings.tags.find(t => t.label === val)?.color || theme.palette.text.secondary;
                                                return <Chip key={val} label={val} size="small" sx={{ bgcolor: alpha(tColor, 0.1), color: tColor }} />;
                                            })}
                                        </Box>
                                    )}
                                >
                                    {settings.tags.map((t) => <MenuItem key={t.id} value={t.label}>{t.label}</MenuItem>)}
                                </Select>
                            </FormControl>
                        </Stack>
                    </Box>
